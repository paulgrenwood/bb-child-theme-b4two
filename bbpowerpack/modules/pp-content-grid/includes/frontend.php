...
if ( ( $settings->layout == 'grid' || $settings->layout == 'grid-packery' ) && $settings->post_grid_filters_display == 'yes' && ! empty( $settings->post_grid_filters ) ) {
...
<div class="pp-posts-wrapper">
	...
	if ( $query->have_posts() ) :
		...
		if ( 'acf_relationship' != $settings->data_source ) {
			// Post filters.
			if ( ( $settings->layout == 'grid' || $settings->layout == 'grid-packery' ) && $settings->post_grid_filters_display == 'yes' && 'none' != $settings->post_grid_filters ) {
	...
	<div class="pp-content-post-<?php if( $settings->layout == 'grid-packery' ): echo $settings->layout . ' pp-content-post-grid'; else: echo $settings->layout; endif; ?><?php echo $css_class; ?> clearfix" itemscope="itemscope" itemtype="http://schema.org/Blog">
			...
			while( $query->have_posts() ) {
				...
				if ( $render ) {
					...
					/*if( 'custom' == $settings->post_grid_style_select ){
						include $module->dir . 'includes/post-module-layout.php';
					}else{
						include apply_filters( 'pp_cg_module_layout_path', $module->dir . 'includes/post-' . $settings->layout . '.php', $settings->layout, $settings );
					}*/
