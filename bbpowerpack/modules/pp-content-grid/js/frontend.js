(function($) {
	...
	window.baseDocumentTitle = document.title;
	console.log(window.baseDocumentTitle);
	...
	PPContentGrid = function(settings)
	{
		...
		if( settings.packery == 'yes' ){
			this.postClass      = this.wrapperClass + ' .pp-content-grid-post';
		}else{
		...
		this.packery		= settings.packery == 'yes' ? true : false;
	...
	PPContentGrid.prototype = {
		...
		_gridLayout: function()
		{
			...
			wrap.imagesLoaded( $.proxy( function() {
				...
				if( window.packery ){
					window.packeryGrid = $(this.nodeClass).find('.pp-content-post-grid-packery').packery(postFilterData);
				}else{
                ...
                if ( this.settings.filters || this.masonry ) {
					...
					filterWrap.on('click', '.pp-post-filter', function() {
						...
						if ('static' === base.filterType) {
							...
							if( window.packery ){
								window.packeryGrid.packery({ filter: filterVal });
							}else{
						...
						//Update URL to current Filter
						if( queryTerm == null || queryTerm == undefined ){
							document.title = window.baseDocumentTitle;
							window.history.replaceState({}, '', `${location.pathname}`);
						}else{
							searchParams.set('filter', queryTerm);
							var termTitle = filterWrap.find('li[data-term="' + queryTerm + '"]').text();
							document.title = termTitle + ' ' + window.baseDocumentTitle;
					...
					// Trigger filter by ?filter= parameter in URL.
					if( searchParams.get('filter') !== null ){
						var filterTerm = searchParams.get('filter');
						filterWrap.find('li[data-term="' + filterTerm + '"]').trigger('click');
					}
                ...
                if( ! this.masonry ) {
                    setTimeout( function() {
						...
						if( window.packery ){
							window.packeryGrid.packery('layout');
						}else{
							wrap.isotope('layout');
						}
		...
		_renderPosts: function (response, args) {
			...
			if ( ( 'load_more' !== self.settings.pagination && 'scroll' !== self.settings.pagination ) || self.isFiltering ) {
				if( window.packery ){
					window.packeryGrid.packery('remove', $(this.postClass) );
				}else{
			...
			if (!this.masonry) {
				if( window.packery ){
					$(this.postClass).addClass('packeryRendered');
					window.packeryGrid.append( $(response.data) ).packery('appended', $(this.postClass).not('.packeryRendered') );
					$(this.postClass).removeClass('packeryRendered');
				}else{
				...
				if( window.packery ){
					$(this.postClass).addClass('packeryRendered');
					wrap.append( $(response.data) ).packery('appended', $(this.postClass).not('.packeryRendered') );
					$(this.postClass).removeClass('packeryRendered');
				}else{
			...
			wrap.imagesLoaded($.proxy(function () {
				setTimeout(function () {
					if (!this.masonry && !window.packery) {
						self._gridLayoutMatchHeight();
					}
					if( window.packery ){
						wrap.packery('layout');
		...
		_initLoadMore: function()
		{
			...
			$(self.nodeClass).on('grid.filter.complete', function() {//debugger;
				if ( $(self.nodeClass).find( '.pp-content-grid-pagination' ).length > 0 ) {
					self._getTotalPages();//debugger;
